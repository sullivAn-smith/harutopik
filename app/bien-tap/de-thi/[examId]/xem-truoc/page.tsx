...
export default async function ExamPreviewPage({ params, searchParams }: { params: Promise<{ examId: string }>; searchParams: Promise<{ error?: string; section?: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-10">
    ...
    <div className="mt-6 space-y-6">{visibleQuestions.map((question, index) => {
      ...
      const sharedQuestions = question.passageBlockKey ? visibleQuestions.filter((item) => item.passageBlockKey === question.passageBlockKey) : [];
      ...
      return <article key={question.id ?? `${question.section}-${question.position}`} className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        {question.section === "reading" && firstOfShared && sharedRange && question.instruction && <p className="mt-4 text-lg font-black text-slate-900">※ [{sharedRange}] {question.instruction}</p>}
        ...
        <div className={`mt-4 ${showSideImage ? "grid items-start gap-6 lg:grid-cols-[minmax(240px,0.8fr)_minmax(360px,1.2fr)]" : ""}`}>
          ...
          <div>{!sharedRange && <p className="text-sm font-bold text-slate-500">{question.instruction}</p>}<h2 className="mt-2 text-xl font-black">{question.prompt}</h2>{question.section === "reading" && question.audioText && <div className="mt-4 rounded-2xl border bg-slate-50 p-4"><p className="text-center text-xs font-black text-slate-500">＜보기＞</p><p className="mt-2 whitespace-pre-wrap font-semibold">{question.audioText}</p></div>}
