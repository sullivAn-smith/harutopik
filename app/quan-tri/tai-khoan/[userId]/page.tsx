  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-10 lg:px-8">
      ...
      <section className="mt-7 grid gap-4 sm:grid-cols-4">
        {[["Role hiện tại", account.role], ["Tiến độ tổng", `${account.progress.overallPercent}%`], ["Bài hoàn thành", `${account.progress.completed}/${account.progress.total}`], ["Đang học", String(account.progress.inProgress)], ["Gói", account.subscription], ["Nội dung đã tạo", String(account.contentCount)]].map(([label, value]) => <article key={label} className="surface-card bg-white p-5"><p className="text-sm font-bold text-ink-600">{label}</p><p className="mt-2 text-xl font-black">{value}</p>{label === "Tiến độ tổng" && <p className="mt-1 text-xs font-semibold text-slate-500">Cập nhật gần nhất: {account.progress.lastStudiedAt ? new Intl.DateTimeFormat("vi-VN", { dateStyle: "medium" }).format(new Date(account.progress.lastStudiedAt)) : "Chưa học"}</p>}</article>)}
      </section>
