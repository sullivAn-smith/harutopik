...
function NotificationLink({ unread = 0 }: { unread?: number }) {
  return (
    <Link
      href="/thong-bao"
      aria-label={
...
async function AdminNotificationLink({ userId }: { userId: string }) {
  const unread = await getUnreadNotificationCount(userId);
  return <NotificationLink unread={unread} />;
}
      ...
      <div className="min-w-0">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/80 bg-white/80 px-5 py-4 backdrop-blur-xl lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <Suspense fallback={<NotificationLink />}>
              <AdminNotificationLink userId={actor.id} />
            </Suspense>
