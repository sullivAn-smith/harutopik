...
import { examDraftSchema, getExamEligibility } from "@/lib/exams/types";
...
export async function hotfixPublishedExam(examId: string, _state: { ok: boolean; message: string }, formData: FormData) {
  ...
  if (!parsed.success) return { ok: false, message: parsed.error.issues[0]?.message ?? "Dữ liệu hotfix không hợp lệ." };
  ...
  const { data, error } = await supabase.rpc("hotfix_published_exam", {
    p_exam_id: examId,
    p_exam: { code: parsed.data.code, title: parsed.data.title, description: parsed.data.description, listeningDurationMinutes: parsed.data.listeningDurationMinutes, readingDurationMinutes: parsed.data.readingDurationMinutes, instructions: parsed.data.instructions },
