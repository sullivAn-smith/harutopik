...
export function HomeClient({
  ...
  useEffect(() => {
    let frame = 0;
    const normalizeDesktopViewport = () => {
    ...
    window.addEventListener("resize", normalizeDesktopViewport);
    return () => {
      cancelAnimationFrame(frame);
      window.removeEventListener("resize", normalizeDesktopViewport);
    };
  }, []);
