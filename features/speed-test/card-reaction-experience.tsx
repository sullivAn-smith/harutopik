...
type SavedResult = { score: number; accuracy: number; correctCount: number; wrongCount: number; bestCombo: number; perfectCount: number; clearedCards: number; revengeCount: number; personalBest: { fastestClear: boolean; highestScore: boolean }; ranking?: { attemptsRemaining?: number } };
...
export function CardReactionExperience({ vocabulary, lessonName, lessonId, courseSlug, lessonSlug, progressById, backHref, rankedMode = false, rankedAttemptsRemaining = 3, onBackToArena }: {
  vocabulary: VocabularyItem[]; lessonName: string; lessonId: string; courseSlug: string; lessonSlug: string;
  progressById: Record<string, SpeedTestWordProgress>; backHref: string; rankedMode?: boolean; rankedAttemptsRemaining?: number; onBackToArena?: () => void;
}) {
  const [stage, setStage] = useState<Stage>("setup");
  const [level, setLevel] = useState<CardReactionLevel>(rankedMode ? "medium" : vocabulary.length >= 16 ? "medium" : "easy");
  ...
  const finish = useCallback((finalAnswers: Answer[], finalRemaining: number) => {
    ...
    void fetch("/api/v1/card-reactions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      attemptId: attemptId.current, courseSlug, lessonSlug, level, direction, mode, ranked: rankedMode,
      ...
      setSaved(payload.data); setScore(payload.data.score); setBestCombo(payload.data.bestCombo);
      if (typeof payload.data.ranking?.attemptsRemaining === "number") setRankedRemaining(payload.data.ranking.attemptsRemaining);
      setSync("saved");
    }).catch(() => setSync("failed"));
  }, [cards, courseSlug, direction, lessonSlug, level, mode, rankedMode]);
  ...
  function start(config: GameConfig = { level, direction, mode }) {
    ...
    const board = createCardReactionBoard({ vocabulary, level: config.level, direction: config.direction, progressById: rankedMode ? {} : progressById });
  ...
  const answer = useCallback((userAnswer: string) => {
    ...
    window.setTimeout(() => {
      ...
      if (!correct) {
        ...
        setStates((current) => ({ ...current, [activeCard.id]: "hidden" }));
        setCards((current) =>
          relocateWrongCard({
            cards: current,
            wrongCardId: activeCard.id,
            clearedCardIds,
          })
        );
      }
  ...
  }, [activeCard, answers, cards.length, cleared, combo, feedback, finish, level, lives, mode, sound, states, weakIds]);

  if (stage === "setup") return <Setup vocabularyCount={vocabulary.length} lessonName={lessonName} level={level} setLevel={setLevel} direction={direction} setDirection={setDirection} mode={mode} setMode={setMode} sound={sound} setSound={setSound} backHref={backHref} onStart={() => start()} onQuickStart={quickStart} rankedMode={rankedMode} rankedRemaining={rankedRemaining} />;
