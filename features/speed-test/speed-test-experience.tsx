...
export function SpeedTestExperience({ lists, initialListId, initialDailyMode = false, fixedSource, progressById = {}, backHref = "/tu-cua-toi", backLabel = "Về bộ từ", challengeDate, dailyCompletedToday = false, dailyBestAccuracy, rankedMode = false, rankedAttemptsRemaining = 3, onBackToArena }: { lists: SpeedList[]; initialListId?: string; initialDailyMode?: boolean; fixedSource?: SpeedTestSource; progressById?: Record<string, SpeedTestWordProgress>; backHref?: string; backLabel?: string; challengeDate: string; dailyCompletedToday?: boolean; dailyBestAccuracy?: number; rankedMode?: boolean; rankedAttemptsRemaining?: number; onBackToArena?: () => void }) {
  const [listId, setListId] = useState(lists.some((list) => list.id === initialListId) ? initialListId! : (lists[0]?.id ?? ""));
  const [direction, setDirection] = useState<SpeedTestDirection>(rankedMode ? "ko_vi" : "vi_ko");
  const [requested, setRequested] = useState<10 | 20 | 30 | "all">(rankedMode || initialDailyMode ? 20 : 30);
  const [dailyMode, setDailyMode] = useState(rankedMode ? false : initialDailyMode);
  ...
  const selectQuestions = useCallback(
    (items: readonly SpeedTestVocabularySnapshot[]) =>
      rankedMode
        ? selectSpeedTestQuestions(items, 20)
        : selectAdaptiveSpeedTestQuestions(
            items,
            dailyMode ? 20 : requested,
            progressById,
          ),
    [dailyMode, progressById, rankedMode, requested],
  );
  ...
  const finish = useCallback((reason: "completed" | "timed_out", finalAnswers = answers, finalSeconds = Math.max(0, Math.ceil(remainingMs / 1000)), finalBest = bestCombo) => {
    ...
    void fetch("/api/v1/speed-tests", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      ...
      remainingSeconds: finalSeconds, bestCombo: finalBest, finishedReason: reason, dailyChallenge: dailyMode, ranked: rankedMode, challengeDate: dailyMode ? challengeDate : undefined, startedAt, finishedAt: new Date().toISOString(),
    }) }).then(async (response) => {
      const payload = await response.json().catch(() => null) as { data?: { ranking?: { attemptsRemaining?: number } } } | null;
      setSyncState(response.ok ? "saved" : "failed");
      if (response.ok && typeof payload?.data?.ranking?.attemptsRemaining === "number") {
  ...
  }, [answers, attemptId, bestCombo, challengeDate, dailyMode, direction, fixedSource, listId, questions, rankedMode, remainingMs, requested, startedAt]);
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#79d7f5,transparent_36rem),linear-gradient(145deg,#0b87c4,#075397)] px-4 py-6 sm:px-6 sm:py-10">
      <section className="mx-auto max-w-6xl">
        <div className="flex items-center justify-between gap-3">
          <Link href={backHref} className="rounded-2xl bg-white/85 px-5 py-3 font-black text-ink-900 shadow-lg">← {backLabel}</Link>
          <span className="rounded-full bg-[#10243e] px-4 py-2 text-sm font-black tracking-wider text-white">⚡ SPEED TEST</span>
        </div>

        {stage === "setup" && <Setup lists={lists} listId={listId} setListId={setListId} direction={direction} setDirection={setDirection} requested={requested} setRequested={setRequested} dailyMode={dailyMode} setDailyMode={setDailyMode} dailyCompletedToday={dailyCompleted} dailyBestAccuracy={dailyBestAccuracy} activeList={activeList} onNext={prepare} lockList={Boolean(fixedSource)} rankedMode={rankedMode} rankedRemaining={rankedRemaining} />}
        {stage === "preview" && activeList && <Preview list={activeList} direction={direction} count={questions.length} adaptive={hasLearningData && !rankedMode} dailyMode={dailyMode} dailyCompletedToday={dailyCompleted} onBack={() => setStage("setup")} onStart={() => start()} rankedMode={rankedMode} rankedRemaining={rankedRemaining} />}
        {stage === "countdown" && <><GameMusicControl enabled={gameMusic.enabled} volume={gameMusic.volume} toggle={gameMusic.toggle} setVolume={gameMusic.setVolume} /><div className="mt-16 text-center text-white"><p className="text-xl font-black uppercase tracking-[.25em]">Sẵn sàng</p><p className="mt-6 text-[10rem] font-black leading-none drop-shadow-xl">{countdown || "GO!"}</p></div></>}
...
function Setup({ lists, listId, setListId, direction, setDirection, requested, setRequested, dailyMode, setDailyMode, dailyCompletedToday, dailyBestAccuracy, activeList, onNext, lockList, rankedMode, rankedRemaining }: SetupProps) {
  return <section className="mt-8 overflow-hidden rounded-[2rem] border border-white/70 bg-white/95 p-6 shadow-2xl sm:p-10">
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      ...
      <Field label="Số câu"><div className="grid grid-cols-4 gap-2">{([10,20,30,"all"] as const).map((count) => <button type="button" disabled={dailyMode || rankedMode} key={count} onClick={() => { setDailyMode(false); setRequested(count); }} className={`rounded-xl px-3 py-3 font-black disabled:cursor-not-allowed ${requested === count ? "bg-brand-600 text-white" : "bg-sky-50 text-brand-700"}`}>{count === "all" ? "Tất cả" : count}</button>)}</div>{dailyMode && <button type="button" onClick={() => setDailyMode(false)} className="mt-2 text-sm font-black text-brand-700">Chuyển sang Speed Test thường</button>}</Field>
    </div>
    <p className="mt-7 text-sm font-black uppercase tracking-wider text-ink-600">Chế độ</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{(["vi_ko","ko_vi"] as const).map((mode) => <button type="button" disabled={rankedMode} key={mode} onClick={() => setDirection(mode)} className={`rounded-2xl border-2 p-5 text-left transition disabled:cursor-not-allowed ${direction === mode ? "border-brand-600 bg-sky-50 shadow-md" : "border-slate-200"}`}><b className="text-xl text-ink-900">{mode === "vi_ko" ? "Việt → Hàn" : "Hàn → Việt"}</b><span className="mt-1 block text-sm text-ink-600">{mode === "vi_ko" ? "Nhìn nghĩa, gõ tiếng Hàn" : "Nhìn tiếng Hàn, gõ nghĩa Việt"}</span></button>)}</div>
    <button type="button" disabled={!activeList?.items.length || rankedRemaining <= 0} onClick={onNext} className="mt-8 w-full rounded-2xl bg-[#10243e] px-6 py-4 text-lg font-black text-white shadow-xl disabled:opacity-50">{rankedRemaining <= 0 ? "Đã hết lượt xếp hạng hôm nay" : "Xem trước →"}</button>
  </section>;
}
...
function Preview({ list, direction, count, adaptive, dailyMode, dailyCompletedToday, onBack, onStart, rankedMode, rankedRemaining }: { list: SpeedList; direction: SpeedTestDirection; count: number; adaptive: boolean; dailyMode: boolean; dailyCompletedToday: boolean; onBack: () => void; onStart: () => void; rankedMode: boolean; rankedRemaining: number }) { const sample = list.items[0]; return <section className="mt-8 rounded-[2rem] bg-white/95 p-7 shadow-2xl sm:p-10"><button onClick={onBack} className="font-black text-brand-700">← Chỉnh thiết lập</button><div className="mt-7 grid gap-6 lg:grid-cols-[1fr_.8fr]"><div><p className="text-sm font-black uppercase tracking-[.18em] text-brand-600">{rankedMode ? "♛ Ranked Challenge" : dailyMode ? "📅 Daily Challenge" : list.name}</p><h1 className="mt-2 text-4xl font-black text-ink-900">{count} câu · 60 giây</h1>{rankedMode && <p className="mt-3 inline-flex rounded-full bg-amber-100 px-4 py-2 text-sm font-black text-amber-900">Còn {rankedRemaining}/3 lượt hôm nay · Lấy điểm tốt nhất</p>}{dailyMode && <p className="mt-3 inline-flex rounded-full bg-emerald-100 px-4 py-2 text-sm font-black text-emerald-800">{dailyCompletedToday ? "Lượt làm lại chỉ cập nhật kỷ lục" : "Hoàn thành để giữ streak hôm nay"}</p>}{adaptive && <p className="mt-3 ml-2 inline-flex rounded-full bg-amber-100 px-4 py-2 text-sm font-black text-amber-800">🧠 Adaptive · Đã xen kẽ từ yếu và từ từng sai</p>}<ul className="mt-5 space-y-3 font-semibold text-ink-600"><li>✓ Đúng +2 giây, sai -1 giây</li><li>✓ Combo 5 và 10 có thưởng thời gian</li><li>✓ Thời gian tối đa 120 giây</li><li>✓ Câu hỏi được trộn lại ở mỗi lượt</li></ul></div><div className="rounded-3xl bg-gradient-to-br from-sky-50 to-cyan-100 p-6 text-center"><p className="text-sm font-black text-brand-700">Ví dụ</p><p className="mt-5 text-3xl font-black text-ink-900">{direction === "vi_ko" ? sample?.vietnamese : sample?.korean}</p><div className="mt-5 rounded-xl bg-white px-4 py-3 text-slate-400">Nhập {direction === "vi_ko" ? "tiếng Hàn" : "tiếng Việt"}...</div></div></div><button onClick={onStart} className="mt-8 w-full rounded-2xl bg-emerald-600 px-6 py-4 text-xl font-black text-white shadow-xl">{rankedMode ? "Bắt đầu lượt xếp hạng ♛" : dailyMode ? "Bắt đầu Daily Challenge 📅" : "Bắt đầu Speed Test ⚡"}</button></section>; }
...
function Game({ question, direction, value, setValue, inputRef, submit, remaining, combo, position, total, feedback, music }: GameProps) {
  ...
  return <section className={`relative mx-auto mt-8 max-w-4xl overflow-hidden rounded-[2.5rem] border border-white/70 bg-white/95 p-6 text-center shadow-2xl sm:p-10 ${feedbackClass}`}>
    ...
    <GameMusicControl enabled={music.enabled} volume={music.volume} toggle={music.toggle} setVolume={music.setVolume} />
    <div className="relative z-10 flex flex-wrap items-center justify-between gap-3"><b className={`rounded-full bg-amber-100 px-4 py-2 text-xl text-amber-800 ${feedback?.delta ? "speed-time-pop" : ""}`}>⚡ {remaining}s</b><b className="rounded-full bg-orange-100 px-4 py-2 text-orange-700">🔥 COMBO {combo}</b></div>
