...
export function SpeedTestArena({
  ...
  if (activeGame === "typing" && typingGame) {
    return <div className="relative"><FloatingLanguageKeyboard /><ArenaTabs activeGame={activeGame} setActiveGame={setActiveGame} hasAudioGame={Boolean(audioGame)} hasFlashGame={Boolean(flashGame)} hasCardGame={Boolean(cardGame)} />{typingGame}</div>;
  }
  ...
  if (activeGame === "audio" && audioGame) {
    return <div className="relative"><FloatingLanguageKeyboard /><ArenaTabs activeGame={activeGame} setActiveGame={setActiveGame} hasAudioGame hasFlashGame={Boolean(flashGame)} hasCardGame={Boolean(cardGame)} />{audioGame}</div>;
  }
  if (activeGame === "audio") {
    return <ReactionLessonPicker lessons={audioLessons} game="audio" onBack={() => setActiveGame("arena")} />;
  }
  if (activeGame === "flash" && flashGame) {
    return <div className="relative"><FloatingLanguageKeyboard /><ArenaTabs activeGame={activeGame} setActiveGame={setActiveGame} hasAudioGame={Boolean(audioGame)} hasFlashGame hasCardGame={Boolean(cardGame)} />{flashGame}</div>;
  }
  if (activeGame === "flash") {
    return <ReactionLessonPicker lessons={audioLessons} game="flash" onBack={() => setActiveGame("arena")} />;
  }
  if (activeGame === "card" && cardGame) {
    return <div className="relative"><FloatingLanguageKeyboard /><ArenaTabs activeGame={activeGame} setActiveGame={setActiveGame} hasAudioGame={Boolean(audioGame)} hasFlashGame={Boolean(flashGame)} hasCardGame />{cardGame}</div>;
  }
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-2">
          <button
            ...
            style={{ backgroundImage: "url('/speed-arena-typing-bg.png')" }}
            className="group relative min-h-72 overflow-hidden rounded-[2rem] border border-white/25 bg-cover bg-center p-7 text-left shadow-2xl transition hover:-translate-y-1 hover:shadow-[0_28px_60px_rgba(3,35,74,.4)] sm:p-9"
          >
          ...
          <button
            ...
            style={{ backgroundImage: "url('/speed-arena-audio-bg.png')" }}
            className="group relative min-h-72 overflow-hidden rounded-[2rem] border border-amber-200 bg-cover bg-center p-7 text-left text-amber-950 shadow-2xl transition hover:-translate-y-1 hover:shadow-[0_28px_60px_rgba(116,76,7,.32)] sm:p-9"
          >
            <p className="text-xs font-black uppercase tracking-[.2em] text-amber-700">Thử thách mới</p>
            <h2 className="mt-2 text-3xl font-black">Audio Reaction</h2><p className="mt-3 max-w-md font-semibold leading-7 text-amber-900/70">Nghe từ và câu ví dụ trong từng bài học. Chọn hoặc gõ đáp án và bảo vệ năm mạng của bạn.</p>
            <span className="mt-6 inline-flex rounded-xl bg-amber-950 px-4 py-2.5 font-black text-amber-50">Chơi Audio Reaction →</span>
          </button>
          <button onClick={() => setActiveGame("flash")} style={{ backgroundImage: "url('/speed-arena-flash-bg.png')" }} className="group relative min-h-72 overflow-hidden rounded-[2rem] border border-emerald-200 bg-cover bg-center p-7 text-left text-emerald-950 shadow-2xl transition hover:-translate-y-1 hover:shadow-[0_28px_60px_rgba(6,95,70,.28)] sm:p-9"><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-700">Nhớ chủ động</p><h2 className="mt-2 text-3xl font-black">Flash Recall</h2><p className="mt-3 max-w-md font-semibold leading-7 text-emerald-900/70">Ghi nhớ từ trước khi nó biến mất, rồi chọn hoặc gõ đáp án trước khi hết giờ.</p><span className="mt-6 inline-flex rounded-xl bg-emerald-950 px-4 py-2.5 font-black text-emerald-50">Chơi Flash Recall →</span></button>
          <button onClick={() => setActiveGame("card")} style={{ backgroundImage: "url('/speed-arena-card-bg.png')" }} className="group relative min-h-72 overflow-hidden rounded-[2rem] border border-fuchsia-200 bg-cover bg-center p-7 text-left text-fuchsia-950 shadow-2xl transition hover:-translate-y-1 hover:shadow-[0_28px_60px_rgba(112,26,117,.3)] sm:p-9"><p className="text-xs font-black uppercase tracking-[.2em] text-fuchsia-700">Dọn board nhanh nhất</p><h2 className="mt-2 text-3xl font-black">Card Reaction</h2><p className="mt-3 max-w-md font-semibold leading-7 text-fuchsia-950/70">Lật thẻ, trả lời đúng để giữ thẻ và clear toàn bộ board trước khi hết giờ hoặc hết mạng.</p><span className="mt-6 inline-flex rounded-xl bg-fuchsia-950 px-4 py-2.5 font-black text-fuchsia-50">Chơi Card Reaction →</span></button>
        </div>
      </section>
    </main>
  );
